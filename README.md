...
[![PyPI](https://img.shields.io/pypi/v/cognis-crmsync.svg)](https://pypi.org/project/cognis-crmsync/)
[![CI](https://github.com/cognis-digital/crmsync/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/crmsync/actions)
...
CRMSYNC exists for one job — bidirectional, idempotent sync of contacts/deals between a local sqlite source-of-truth and crm apis (hubspot/pipedrive/salesforce) via one config. — and does it without a SaaS bill or heavyweight setup.
...
```bash
crmsync --version
crmsync scan .                      # scan the current project
crmsync scan . --format json
crmsync scan . --fail-on high       # non-zero exit for CI gates
crmsync mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **Airbyte/Singer taps, in the spirit of Grouparoo**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`crmsync` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
