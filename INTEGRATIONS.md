...
**crmsync** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `crmsync` emits JSON findings, pipe them straight to a destination — `--dry-run`
...
crmsync ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
